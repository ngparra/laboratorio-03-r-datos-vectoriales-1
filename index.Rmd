...
3. (5%) Mediante la función ```kable()```, despliegue en una tabla las diez primeras observaciones y las variables correspondientes a nombre del país, continente, población, esperanza de vida y renta per cápita del data frame  ```world```. Despliegue los nombres de las columnas en español y un título para la tabla.
```{r}
kable(
...
kable(
  world.ordenadoxpais[, c("name_long","continent","pop","lifeExp","gdpPercap")], 
  col.names=c("País","Continente","Población","Esperanza de vida","Renta per cápita"),
  caption="Datos demográficos de los países del mundo"
  ) %>%
kable_styling() %>%
...
5. (5%) Filtre el data frame ```world``` para extraer en otro data frame las observaciones correspondientes a los siete países de Centroamérica. Mantenga las mismas columnas de los ejercicios anteriores y despliegue los datos en una tabla.
```{r}
...
# Se formatea la tabla con el nuevo data frame
kable(
  centro_america[, c("name_long","continent","pop","lifeExp","gdpPercap")], 
  col.names=c("País","Continente","Población","Esperanza de vida","Renta per cápita"),
...
6. (25%) Con la función ```ggplot()``` del paquete [ggplot2](https://ggplot2.tidyverse.org/), genere un gráfico de barras que muestre la esperanza de vida de los siete países de Centroamérica. El gráfico debe:
```{r}
```

7. (25%) Con la función ```plot()``` del paquete [sf](https://cran.r-project.org/web/packages/sf/index.html), genere un [mapa de coropletas](https://en.wikipedia.org/wiki/Choropleth_map) con colores que muestren la esperanza de vida de los siete países de Centroamérica. El mapa debe llevar un título.
```{r}
```

8. (25%) Extraiga en un nuevo data frame los países de Centroamérica con esperanza de vida mayor o igual a 75 años y genere el mapa de coropletas y su título correspondientes.
```{r}
```