...
3. Generar documentos en R Markdown con tres tipos de visualizaciones de datos:
  - Tablas.
  - Gráficos.
  - Mapas.
...
```{r}
kable(world[1:10, 1:8], caption="Países del mundo")
```

3. (5%) Mediante la función ```kable()```, despliegue en una tabla las diez primeras observaciones (i.e. filas, registros) y las variables correspondientes a nombre del país, población, esperanza de vida y de renta per cápita del data frame  ```world```.
```{r}
kable(
  world[1:10, c("name_long", "pop", "lifeExp", "gdpPercap")], 
  col.names = c("País", "Población", "Esperanza de vida", "Renta per cápita"),
